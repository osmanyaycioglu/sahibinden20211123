...
public class ClientConfig {
    ...
    public static class MyRestClientErrorHandler implements ResponseErrorHandler {
        ...
        public boolean hasError(final ClientHttpResponse responseParam) throws IOException {
            return responseParam.getStatusCode()
                                .is4xxClientError();
        }
        ...
        public void handleError(final ClientHttpResponse responseParam) throws IOException {
            if (ClientConfig.logger.isInfoEnabled()) {
                ClientConfig.logger.info("[ClientConfig.MyRestClientErrorHandler][handleError]-> Error Status code : "
                                         + responseParam.getStatusCode());
            }
        }
    }
    ...
    public static class MyRestClientInterceptor implements ClientHttpRequestInterceptor {
        ...
        public ClientHttpResponse intercept(final HttpRequest requestParam,
            ...
            ClientHttpResponse executeLoc = executionParam.execute(requestParam,
                                                                   bodyParam);
            if (ClientConfig.logger.isInfoEnabled()) {
                ClientConfig.logger.info("[ClientConfig.MyRestClientInterceptor][intercept]-> Call Delta : "
        ...
        try {
            ...
            for (Header headerLoc : allHeadersLoc) {
                ...
                if ("Keep-Alive".equals(nameLoc)) {
                    String[] splitLoc = headerLoc.getValue()
                                                 .split("=");
            ...
            ClientConfig.logger.error("[ClientConfig][connectionKeepAliveStrategy]-> *Error* : " + eLoc.getMessage(),
                                      eLoc);
        }
        return 60_000;
